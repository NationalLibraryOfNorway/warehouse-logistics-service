...
```bash
cd docker
sudo <docker|nerdctl> compose exec keycloak /opt/keycloak/bin/kc.sh export --file "/tmp/mlt-local-realm-export.json" --users "same_file" --realm "mlt-local"
sudo <docker|nerdctl> compose cp keycloak:/tmp/mlt-local-realm-export.json ./keycloak/import/mlt-local-realm-export.json
sudo chown $USER:$USER "./keycloak/import/mlt-local-realm-export.json" # Ensure user owns the file
