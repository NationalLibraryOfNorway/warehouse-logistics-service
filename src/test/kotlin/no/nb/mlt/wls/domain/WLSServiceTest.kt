...
class WLSServiceTest {
    ...
    private val emailRepoMock = mockk<EmailRepository>()
    private val mailSenderMock = mockk<JavaMailSender>()
    ...
    fun `addItem should save and return new item when it does not exists`() {
        ...
        val cut = WLSService(itemRepoMock, orderRepoMock, storageSystemRepoMock, inventoryNotifierMock, emailRepoMock, mailSenderMock)
        runTest {
            val itemResult =
                cut.addItem(
                    ItemMetadata(
    ...
    fun `createOrder should save order in db and storage system`() {
        ...
        coEvery { mailSenderMock.createMimeMessage() } answers { createFakeMail() }
        coEvery { mailSenderMock.send(any() as MimeMessage) } answers { createFakeMail() }
    ...
    private fun createFakeMail(): MimeMessage {
        return MimeMessage(null as Session?)
    }
}
